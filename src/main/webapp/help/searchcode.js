	...
	for(i=0;i<hitsIndex.length;++i) {
	  ...
	  if(fileLine=="undefined") {
		...
		if (textIndex < fileLine.length)
	    		...
	    		txtStr = "";
                var tt = 'index.htm?'+fileLine.substring(0,fileLine.indexOf(TitleMarker)-1)+'&ST='+searchText;

                strA.push('<p class="bodycopy"><strong>'+(i+1)+'.</strong> <a href="'+pth+'"><strong>'+ttl+'</strong></a><br>');
                if(sum)
                  strA.push(txtStr);
                strA.push('</p>');
    	  }	

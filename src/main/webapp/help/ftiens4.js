...
{ 
  ...
  if (this.level>0) 
  {
    nodeName = ICONPATH + 'sblank.gif';
    leftSide = leftSide + "<td valign=top>"+"<a>" +"<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' alt='' width=1 height=1 border=0></a></td>"
//    leftSide = leftSide + "<td valign=middle>"+"<a>" +"<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=1 height=1 border=0></a></td>"
  }
 ...
 //   { 
    ...
    //  leftSide = leftSide + "<td valign=top>"+"<a>" +"<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>"
  // } else {
      //leftSide = leftSide + "<td valign=top>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>"
    //  leftSide = leftSide + "<td valign=top>"+"<a>" +"<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>"
//}
...
if(this.getID()!=foldersTree.getID()){
  docW = docW + "<tr>" + leftSide + "<td valign=top>";
 // docW = docW + "<tr>" + leftSide + "<td valign=middle>";
  if (USEICONS)
  {
    docW = docW + this.linkHTML(false) 
    docW = docW + "<img id='folderIcon" + this.id + "' name='folderIcon" + this.id + "' src='" + this.iconImageSrc() + "' alt='expand/collapse topic' border=0></a>"
  }
  else
  {
	  if (this.prependHTML == "")
        docW = docW + "<img src=" + ICONPATH + "ftv2blank.gif alt='' height=2 width=2>"
  }
  if (WRAPTEXT)
...
{   
  var i=0 
if(folder.getID()!=foldersTree.getID()){
...
{ 
  ...
  { 
    ...
    if (isTextLink) {
	if (USEFRAMES)
	  docW = docW + "<a href='" + this.hreference + "' TITLE='"+this.desc+"' TARGET=\"contentiframe\" "
	else
	  docW = docW + "<a href='" + this.hreference + "' TARGET=_top "
    }
    else
	docW = docW + "<a href='javascript:void(0)' TARGET=\"contentiframe\" "
    ...
    if (browserVersion > 0) 
      docW = docW + "onClick='javascript:clickOnFolder(\""+this.getID()+"\",\""+isTextLink+"\")'"
...
//      leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2lastnode.gif' width=16 height=22></td>"
 ...
 //leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2blank.gif' width=16 height=22></td>"
  ...
  if (USEICONS)
      docW = docW + "<a " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<img id='itemIcon"+this.id+"' " + "src='"+this.iconSrc+"' alt='open topic' border=0>" + "</a>"
  else
	  if (this.prependHTML == "")
        docW = docW + "<img src=" + ICONPATH + "ftv2blank.gif alt='' height=2 width=3>"
  ...
  if (USETEXTLINKS) 
    docW = docW + "<a " + fullLink + " title=\""+this.desc+"\" id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>"
  else 
    docW = docW + this.desc
...
function leftSideHTML(leftSideCoded) {
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=top background=" + ICONPATH + "ftv2blank.gif><img src='" + ICONPATH + "ftv2blank.gif' alt='' width=16 height=14></td>"
		}
		if (leftSideCoded.charAt(i) == "0")
		{
			retStr = retStr + "<td valign=top><img src='" + ICONPATH + "ftv2blank.gif' alt='' width=16 height=14></td>"
		}
	}
	return retStr
}
...
function clickOnFolder(folderId,fromText) 
{ 
 ...
 // }
  ...
  if (fromText=='true' && isLinked(clicked.hreference)) {
...
function isLinked(hrefText) {
    ...
    result = (result && hrefText.indexOf('javascript:void(0)') < 0);
    return result;
}

function clearLastHighlightetObjLink()
{
  if (lastClicked != null) {
...
//	auxImg.src = ICONPATH + "ftv2lastnode.gif";
	...
	auxImg.src = ICONPATH + "sblank.gif";
}

//Open some folders for initial layout, if necessary
function setInitialLayout() {
  if (browserVersion > 0 && !STARTALLOPEN)
    clickOnNodeObj(foldersTree);//MYCOMMENT -- remove to close all
